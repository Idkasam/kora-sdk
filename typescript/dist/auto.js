...
exports.KoraAuto = void 0;
const DEFAULT_BASE_URL = 'https://api.koraprotocol.com';
class KoraAuto {
    scanToken;
    baseUrl;
    warnThrottle = new Map();
    constructor(config) {
        this.scanToken = config.scanToken;
        this.baseUrl = (config.baseUrl ?? DEFAULT_BASE_URL).replace(/\/$/, '');
    }
    async observe(params) {
        const svc = params.serviceName ??
            process.env.KORA_SERVICE_NAME ??
            (await import('node:os')).hostname();
        const env = params.environment ?? process.env.KORA_ENVIRONMENT ?? 'unknown';
        const rtId = params.runtimeId ?? process.env.KORA_RUNTIME_ID ?? 'unknown';
        const body = {
        ...
        try {
            ...
            const errorReason = err instanceof Error ? err.constructor.name.toLowerCase() : 'unknown';
            this.warn(params.vendor, svc, env, rtId, errorReason);
            return { status: 'error' };
        }
    }
    warn(vendor, service, env, runtimeId, error) {
        ...
        process.stderr.write(`KORA_SCAN_WARN vendor=${vendor} service=${service} env=${env} runtime=${runtimeId} error=${error}\n`);
    }
}
exports.KoraAuto = KoraAuto;
