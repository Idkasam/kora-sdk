...
import {
  ...
  canonicalize,
  sign,
  verifySeal as verifySealCrypto,
  buildSignedFields,
} from './crypto.js';
...
export class Kora {
  ...
  async authorize(
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      ...
      const canonical = canonicalize(signedFields);
      ...
      try {
        ...
        if (result.decision === 'DENIED' && result.traceUrl) {
          console.error(
            `[kora] DENIED: ${result.reasonCode} — trace: ${this.baseUrl}${result.traceUrl}`,
          );
        }
        ...
        const isNetworkError =
          ...
          (err instanceof Error &&
            ('code' in err &&
              ['ECONNREFUSED', 'ETIMEDOUT', 'ECONNRESET', 'UND_ERR_CONNECT_TIMEOUT'].includes(
  ...
  verifySeal(result: AuthorizationResult, koraPublicKey: string): boolean {
    if (!result.notarySeal) return false;
    ...
    return verifySealCrypto(decisionPayload, seal.signature, koraPublicKey);
  }
...
function parseResponse(raw: RawApiResponse): AuthorizationResult {
  const decision = (raw.decision ?? raw.status ?? 'DENIED') as 'APPROVED' | 'DENIED';
...
function parseSeal(raw: RawNotarySeal): NotarySeal {
  return {
...
function parseLimits(raw: RawLimits): Limits {
  return {
...
function parsePaymentInstruction(raw: RawPaymentInstruction): PaymentInstruction {
  return {
...
function parseDenial(raw: RawDenialObject): DenialObject {
  return {
    reasonCode: raw.reason_code,
    message: raw.message,
    hint: raw.hint,
...
function parseTrace(raw: RawEvaluationTrace): EvaluationTrace {
  return {
